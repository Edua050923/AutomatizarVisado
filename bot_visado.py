...
class BotVisado:
    ...
    def enviar_resumen_12h(self):
        try:
            now = datetime.now()
            # Este es el objeto datetime que pasaremos a la DB
            cutoff_time_obj = now - timedelta(hours=self.resumen_interval_hours) 
            ...
            for c in self.cuentas:
                ...
                if self.db:
                    try:
                        # ¡AQUÍ ESTÁ EL CAMBIO!
                        # Pasamos el cutoff_time_obj directamente a la función de la DB.
                        # Limitamos a 100, más que suficiente para un resumen.
                        ciclos_recientes = self.db.cargar_historial(
                            ident, 
                            limite=100, 
                            cutoff_time=cutoff_time_obj
                        )
                    except Exception as e:
                        self._log('warning', ident, f"Error cargando historial DB: {e}")
                else:
                    # Modo simulado (ya filtraba por tiempo, así que está bien)
                    ciclos_recientes = self.cargar_historial(ident, self.resumen_interval_hours)

                # ¡YA NO NECESITAMOS FILTRAR AQUÍ!
                # La base de datos o el modo simulado ya nos dieron 
                # solo los ciclos del período de resumen.
            ...
            # Usamos el objeto cutoff_time_obj para formatear el string
            resumen_texto = f"Resumen desde {cutoff_time_obj.strftime('%Y-%m-%d %H:%M:%S')} hasta {now.strftime('%Y-%m-%d %H:%M:%S')}. Total ciclos: {total_ciclos} - Exitosa: {total_exitosos} - Fallidas: {total_fallidos}"
